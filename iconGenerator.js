...
class IconGenerator {
  constructor() {
    ...
    this.sizes = flagIndex === -1 ? this.defaultSizes : process.argv.slice(flagIndex+1).map(Number);
  ...
  createImage(size) {
    ...
    // Generate random RGB values if the --random flag is specified, otherwise use default values
    const r = this.randomFlag ? Math.floor(Math.random() * 128) + 127 : this.defaultR;
    const g = this.randomFlag ? Math.floor(Math.random() * 128) + 127 : this.defaultG;
    const b = this.randomFlag ? Math.floor(Math.random() * 128) + 127 : this.defaultB;
